class precompute:
    ...
    def precompute_orthogonal_moves(self):
        ...
        for piece_square in range(8):  # 0 to 7 for each square in a rank
            for occupants in range(128):  # 2^7 configurations (excluding the piece itself)
                ...
                occupants_mask |= occupants & ((0b11 << piece_square) - 1)
                ...
                file_moves[(piece_square, self.rotate_90(occupants_mask))] = self.rotate_270(moves)
    ...
    def precompute_diagonal_moves(self):
        ...
        for square in range(64):
            # Compute diagonal and antidiagonal masks for this square
            diagonal_mask = self.compute_diagonal_mask(square)
            antidiagonal_mask = self.compute_antidiagonal_mask(square)

            for blockers in range(1 << self.count_bits(diagonal_mask)):  # Iterate over possible blocker configurations
                moves = 0
                diagonal_moves[(square, blockers & diagonal_mask)] = moves
                antidiagonal_moves[(square, blockers & antidiagonal_mask)] = moves
    ...
    def precompute_diagonal_moves(self):
        ...
        for square in range(64):
            ...
            for blockers in range(1 << (diagonal_length - 1)):
            ...
            for blockers in range(1 << (antidiagonal_length - 1)):
