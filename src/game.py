...
class Board:
    ...
    def initialize_attack_map(self):
        """Initializes attack maps to expected values based on new game starting position."""
    ...
    def get_attack_map_for_pieces(self, piece_type, color, pieces, board = None):
        ...
        while pieces:
                ...
                for move in movegen.get_scope_from_square(board, square, color, piece_type):
                    attack_map |= move
                pieces &= pieces - 1
        return attack_map
    ...
    def get_attack_map_for_sliding_pieces(self, piece_type, color, pieces, occupants = None):
        ...
        while pieces:
                square = pieces & -pieces
                for move in movegen.generate_sliding_scope(occupants, from_square, color, piece_type):
                    attack_map |= move
                pieces &= pieces - 1
        return attack_map
    ...
    def update_consolidated_attack_maps(self, board = None):
        """Returns consolidated attack maps."""
        if board is None:
            board = self
